...
object StemmerEngine extends Stemmer with Logic {

  import Condition._

  protected[porter] object Steps {
    val step1a = List(
      Rule("SSES", "SS"),
      Rule("IES", "I"),
      Rule("SS", "SS"),
  ...
  protected[porter] def applyRules(word: Word, rules: List[Rule]): Option[Word] = {
  ...
  protected[porter] def applyStep(word: Word, rules: List[Rule]): Word =
    applyRules(word, rules) match {
      case None => word
      case Some(w) => w
    }
  ...
  def stem(word: String): String = {
    try {
      ...
      case t: Throwable =>
        System.err.println(s"Could not process '$word', ${t.getMessage}")
        word
    }
  }
}